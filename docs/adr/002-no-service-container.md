...
## Consequences
- **Every dependency is traceable from source.** An agent or developer can follow `App::boot()` → `RouteRegistrar::register()` → `new HomeController($Blade)` without consulting a service provider or config file.
