...
## Decision
Encode architectural rules as 51 custom Rector rules in `utils/rector/src/`. Rules run as part of `./run check:all` and the pre-commit hook. Each rule has a `mode` ('auto' for auto-fix, 'warn' for TODO annotation) and a descriptive `message`.
...
- **AI agents get immediate feedback.** An agent writes code, runs `./run check:all`, and Rector either fixes the code or explains what's wrong via TODO comments.
