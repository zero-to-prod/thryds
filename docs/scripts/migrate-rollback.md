# Fix: migrate-rollback.php — Externalize Hardcoded Namespace and Path

## Script

`scripts/migrate-rollback.php`

## Violations

### 1. Hardcoded migrations namespace (line 25)

```php
migrations_namespace: 'ZeroToProd\\Thryds\\Migrations\\',
```

### 2. Hardcoded migrations directory (line 24)
...
Load `migrations-config.yaml` (shared with `check-migrations.php`, `generate-migration.php`, `migrate.php`, `migrate-status.php`):
