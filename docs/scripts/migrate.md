# Fix: migrate.php — Externalize Hardcoded Namespace and Path
...
### 1. Hardcoded migrations namespace (line 28)

```php
migrations_namespace: 'ZeroToProd\\Thryds\\Migrations\\',
```

### 2. Hardcoded migrations directory (line 27)
...
Load `migrations-config.yaml` (shared with `check-migrations.php`, `generate-migration.php`, `migrate-status.php`, `migrate-rollback.php`):
